...
export function ArchitectureTab({
  ...
  const missingArtifacts = manifest.artifacts.filter((a) => !a.present);
  const fieldAudit = manifest.field_audit ?? [];
  const fieldMissing = fieldAudit.filter((f) => !f.present).length;

  return (
    <div className="space-y-4">
      {/* ============ 本页结论 ============ */}
      <Verdict
        what="本页在证明：这套系统是怎么搭的，以及「它真的在算，不是放录像」"
        conclusion={
          <>
            一句 brief 依次经过<b className="text-slate-900">六个 Agent</b>：读需求 → 拉候选 → 四层门禁筛人 → 语义适配打分 → 分预算 → 拿真值反过来审计自己，
            每一步的耗时都是本次运行的实测值。同一套判定规则<b className="text-slate-900">用 Python 和 TypeScript 各写了一遍</b>，
            对同一份 5,000 人数据逐字段比对（连中文理由都逐字符比）—— 差异 {c ? int0(c.verdict.diff_count) : '—'} 条。
          </>
        }
      ...
      <Panel
        ...
        subtitle="每个 Agent 的耗时是本次运行的实测值（performance.now()）；颜色代表真实性类别，不是装饰：蓝=纯规则真算，靛=构建期真调 LLM·结果固化，紫=以 gt 为裁判的审计"
      >
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          ...
          <span className="muted">A3 内部：四层按顺序执行，任一层给出 block 即短路（展开下面「四层门禁」看每层管什么）</span>
        </div>
      </Panel>
      ...
      {/* ============ 明细全部折叠 ============ */}
      <Panel title="工程明细（可逐项核对，默认收起）" subtitle="四层门禁、逐臂核对、数据流与真值边界、产物 sha256 清单、字段自检、性能账" bodyClass="space-y-2">
        <Collapse
          flag="detail"
          count={4}
          title="A3 四层门禁分别管什么：G0 数据完整性 2 条 / G1 真实性 7 条 / G2 一致性 6 条 / G3 品牌安全 5 条，任一层 block 即短路"
          hint="本次运行每层命中人数由浏览器现算"
        >
        ...
        {c && (
          <Collapse
            flag="evidence"
            count={(c.budget.arms ?? []).length}
            title={`${int0((c.budget.arms ?? []).length)} 个分配臂逐项核对：人数/条数、花费、CPM 全部逐字段比对，外加阈值文件版本与数据集 sha256`}
            hint="读 consistency.json 的 arms 数组；含 Python 侧只落摘要的臂的说明与产物自带的诚实声明"
          >
                      ...
                      <tr key={`${a.campaign_id}-${a.arm}`} className="hairline">
                        ...
                        <td className="td text-[11px] text-slate-600">
                          ...
                          {a.per_person_compared === false && (
                            <span className="ml-1 text-[10px] text-amber-700" title="Python 侧只落摘要（无逐人明细），本臂只比摘要标量、约束检查与 trace">
                              仅摘要
                            </span>
                          )}
                        </td>
        ...
        <Collapse
          flag="evidence"
          count={6}
          title="数据流与真值边界：谁能读 gt、谁不能，在架构上就分开了（门禁/预算/LLM 三层由一条 AST 静态扫描测试守着，一次 gt 访问都不许有）"
          hint="datagen → Python 标定 → Python 判定/预算/审计 → 构建期 LLM runner → prepare-data → 浏览器 TS 引擎"
        >
        ...
        <Collapse
          ...
          title={`前端消费的产物清单：${manifest.artifacts.length} 个产物逐个记录来源文件、源/上线字节数与 sha256${missingArtifacts.length > 0 ? `，其中 ${missingArtifacts.length} 个未生成（照实标黄，不用占位数字冒充）` : '，全部在位'}`}
          hint="manifest.json 由 prepare-data.mjs 生成；上线数据合计体积见下面的数据集元信息"
        >
                ...
                {manifest.artifacts.map((a) => (
                  <tr key={a.key} className="hairline">
                    <td className="td num text-[11px]">
                      ...
                      {a.note && (
                        <Hint text={a.note}>
                          <span className="ml-1 text-[10px] text-live-600">ⓘ</span>
                        </Hint>
                      )}
                    </td>
                    <td className="td num text-[11px] text-slate-500">{a.source}</td>
                    ...
                    <td className="td num text-[11px] text-slate-500" title={a.source_sha256 ?? ''}>
        ...
        {fieldAudit.length > 0 && (
          <>
            <div className="mt-3 text-[12px] font-medium text-slate-800">
              关键字段在位自检：{fieldAudit.length} 个字段
              {fieldMissing > 0 ? `，其中 ${fieldMissing} 个缺失` : '，全部在位'}
            </div>
            <div className="muted mb-1.5">
              prepare-data 对可选产物<b className="text-slate-600">整份原样搬运、不做字段白名单</b>，
              ...
              {fieldAudit.map((f) => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="num truncate text-[11px] text-slate-600">
                      {f.artifact}.{f.path}
                    </div>
        ...
        <Collapse
          flag="detail"
          count={Object.keys(manifest.dataset.injection_actual_rates).length}
          title={`数据集与阈值元信息 + 注入率核对：连「注入了多少水号」都不靠记忆，直接读 manifest（数据集 ${int0(manifest.dataset.n)} 人 · 种子 ${manifest.dataset.seed} · sha256 ${manifest.dataset.sha256.slice(0, 10)}…）`}
          hint="声明注入率 vs 实际落盘比率逐类对照；阈值分组与回退格子数同页"
        >
        ...
        {result && (
          <Collapse
            ...
            title={`本次运行的性能账：端到端 ${ms(result.totalMs)}，六个阶段逐个计时（performance.now() 实测，不是估算）`}
            hint={`整份数据集 ${compact(manifest.dataset.n)} 条全量在浏览器内存里，没有分页请求后端`}
          >
            <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-6">
              {result.stages.map((s) => (
                <div key={s.id} className="card px-2.5 py-2">
                  <div className="num text-[11px] text-slate-500">{s.id}</div>
