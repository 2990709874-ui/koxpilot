...
export function TruthChip({ kind }: { kind: 'rule' | 'llm-offline' | 'audit' | 'synthetic' | 'python' }) {
  const map: Record<string, [string, string]> = {
    rule: ['浏览器内真算', 'border-live-200 bg-live-50 text-live-700'],
    'llm-offline': ['构建期真调 LLM · 结果固化', 'border-indigo-200 bg-indigo-50 text-indigo-700'],
    audit: ['以 gt 为裁判的审计', 'border-fuchsia-200 bg-fuchsia-50 text-fuchsia-700'],
    synthetic: ['合成数据（固定种子）', 'border-slate-200 bg-slate-50 text-slate-700'],
    python: ['Python 离线全量', 'border-emerald-200 bg-emerald-50 text-emerald-700'],
  };
      ...
      {options.map((o) => (
        <button
          ...
          className={`rounded-md ${size === 'sm' ? 'px-2 py-1 text-[11px]' : 'px-3 py-1.5 text-[12px]'} transition-colors ${
            value === o.value ? 'bg-live-100 text-live-700 shadow-[inset_0_0_0_1px_rgba(34,211,238,0.25)]' : 'text-slate-600 hover:text-slate-800'
          }`}
        >
          {o.label}
        </button>
...
export function Toggle({
  ...
  return (
    ...
    >
      <span
        className={`relative h-4 w-8 rounded-full transition-colors ${checked ? 'bg-live-500' : 'bg-slate-200'}`}
      >
        <span
...
export function Hint({ text, children }: { text: React.ReactNode; children: React.ReactNode }) {
  return (
    <span className="group relative inline-flex cursor-help items-center">
      {children}
      <span className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 hidden w-72 -translate-x-1/2 rounded-lg border border-slate-200 bg-white p-2.5 text-left text-[11px] leading-relaxed text-slate-700 shadow-xl backdrop-blur group-hover:block">
        {text}
      </span>
    </span>
  );
...
export function Bar({
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-100" style={{ height }}>
        <div className="h-full rounded-full transition-all duration-500" style={{ width: `${w}%`, background: color }} />
      </div>
      {label && <span className="num shrink-0 text-[11px] text-slate-600">{label}</span>}
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-dashed border-amber-200 bg-amber-50 p-4">
      ...
      <p className="muted mt-1.5">
        缺少产物 <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-[10px] text-amber-700">{file}</code>
        。本板块按缺失降级显示，<b className="text-amber-700/90">不会用估算值或占位数字冒充</b>。
      </p>
...
export function Drawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      <div className="flex-1 bg-slate-500 backdrop-blur-[2px]" onClick={onClose} />
      <aside className={`${width} overflow-y-auto border-l border-slate-200 bg-white shadow-2xl`}>
        <header className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b border-slate-200 bg-white px-4 py-3 backdrop-blur">
          ...
          <button
            onClick={onClose}
            className="rounded-md border border-slate-200 px-2 py-1 text-[11px] text-slate-600 hover:border-slate-300 hover:text-slate-800"
          >
            关闭 Esc
          </button>
        </header>
...
export function KV({ k, v, mono = true }: { k: React.ReactNode; v: React.ReactNode; mono?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-3 py-1">
      ...
      <span className={`${mono ? 'num' : ''} text-[12px] text-slate-800`}>{v}</span>
    </div>
  );
}
...
export function Note({ children, tone = 'muted' }: { children: React.ReactNode; tone?: 'muted' | 'warn' | 'good' }) {
  const cls =
    tone === 'warn'
      ...
      : tone === 'good'
        ? 'border-emerald-200 bg-emerald-50 text-emerald-800/90'
