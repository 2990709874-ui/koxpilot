...
这条结论对"要不要上 LLM"这个问题很关键：**"把任务丢给大模型"本身不产生价值**。v1 花了 43 万 token，换来一个比 0 token 的规则更差的结果。如果我只做了 v1 就下结论，正确的结论会是"LLM 不适合这个任务"——而那是错的。
...
### 4.3 结论三：v3 的增益无法严格归因，我在产物里标了警告

`prompt_bench.json` 里 v3 的 `change` 字段结尾写着：

> ⚠️ **本版含两处改动，v2→v3 的增益无法在二者之间做严格归因，如实记录。**

两处改动是"改写业务判据"和"注入完整邻接表"。要严格归因，需要再跑两个 arm（只改判据 / 只加邻接表），我没跑。所以正确的说法是"**这两处改动合计带来 +0.2016 的 avg F1**"，而不是"业务判据的重要性大于邻接表"。

我更愿意在产物里留一个 ⚠️，也不愿意在文档里编一个漂亮的归因故事。
...
顺带说一个必须坦白的点：`prompt_variants.py` 的注释称 v3 与生产 prompt 保持一致，但两者**字符数不同**（v3 system 1,607 字符，生产 `_tag_system()` 1,553 字符；user message 一致）。所以横评里 v3 的表现应读作"**≈ 生产版**"，不能读成"= 生产版"。登记在 [05 §10](05-boundaries.md#10-已知的文档代码不一致我自己查出来的没修的都在这)。
...
**外推不是实测。** 5,697,000 这个数字是拿 6,415.5 tokens/call 乘以 888 次得到的，它假设"每次调用的 token 消耗相同"——这个假设在不同任务间明显不成立（brief 2,594 token/item vs tag 299 token/item）。所以我给它打了 `extrapolated=true`，而实测账那一块是 `extrapolated=false`。
