...
贪心的凹性前提在 `decay ∈ (0,1]` 时成立（代码里有断言），但**叠加 5 条结构约束后，"贪心 + 修正"不再保证最优**，我也没有做 MILP 对照，所以说不出"离最优差多少"。另外 `POST_DECAY_SCAN=(0.5,0.7,0.9)` 这个常量定义了但**扫描结果没进 metrics**——decay=0.7 这个关键参数目前无敏感性证据。都登记在 [05 §7](05-boundaries.md#7-边界六预算分配是启发式不是最优解)。
...
两条臂**共享同一批 `GateResult` 与同一套定向筛选**，唯一差异是选人依据：

- **KOXPilot 臂**：过门禁（只收 pass）+ 按边际性价比分配。
- **基线臂**：拿到完全一样的候选池（含 review / reject），只按**粉丝量降序**买到花完。

这样差额能干净地归因到"质量门禁 + 价值排序"，而不是归因到定向口径不同。

审计纪律（`eval/audit.py` 里写死）：**浪费金额与有效曝光一律按 `gt` 算，绝不用引擎自己的分数**。否则就是"我说他是水号，所以我省了钱"的自我表扬。

结果：三个 campaign 合计 $245,000，少浪费 **$79,718.55（占总预算 32.54%）**、有效曝光 **+98.64%**。

但这个数字有两处必须自己先说的水分：

- **基线只选了 8 / 4 / 6 个人**（都是最贵的头部号），而 KOXPilot 选了 69 / 39 / 73 个人。所以增量里混着"分散投放 vs 集中押注"的效应，不能全部归因到质量门禁。
- **BRIEF-002 是负例**（多浪费 $5,117.70、有效曝光 −2.3%），根因是基线那 4 个头部号**恰好都是真号**（浪费率 0.84%），而 KOXPilot 的 39 人里有 3 个水号（12.21%）。对照 BRIEF-001 基线 8 人里有 5 个水号（浪费率 66.98%）——**n=4 的基线方差极大**。
