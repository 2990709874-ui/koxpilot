...
**结论：继续走规则版（方案 B）。而且这个结论不是我写死的一句话，是三条现算判据判出来的**（阈值 `COVERAGE_MIN_FOR_PROMOTION = 0.90` / `DOUBLE_COUNT_MAX_FOR_PROMOTION = 0.20`，判据全过时产物里的 `decision` 会自动变成 `llm_fit_promotable`）：
...
**这个 $735 不构成"LLM 更好"的证据**，我也没把它写进任何招牌数字：样本只有 33 人、方向在 campaign 之间就翻转、而且是在单向罚分的偏置下拿到的。它能支撑的唯一结论是"注入的影响主要不在判定翻转上，而在排序权重上"。
...
- `POST_DECAY_SCAN = (0.5, 0.7, 0.9)` 这个常量在 `budget/policy.py` 里定义了很久，但**扫描结果一直没有进产物**——也就是 decay=0.7 这个采购模型里的关键参数长期**没有任何敏感性证据**。现在补上了：`metrics.json.budget_decay_sensitivity`（`eval/decay_scan.py`，三档共用同一批 GateResult，逐档重跑三臂 + gt 审计）。结论分两半，两半都得说：
  ...
  - 这条扫描**回答不了**"哪一档更接近真实世界"——那需要真实投放的重复触达数据，本项目没有。谁把它读成"我们验证了 0.7 是对的"，就是过度解读；产物里 `assumption_kind` 与 `note` 两个字段专门堵这个读法，并有测试钉住。
