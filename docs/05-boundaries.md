...
2. **基线弱得超出我原本的预期，这反而削弱了结论。** 「按粉丝量降序买到花完」在三个 campaign 里分别只选了 **8 / 4 / 6 个人**（都是最贵的头部号），而 KOXPilot 臂选了 **69 / 39 / 73 个人**。所以 +98.64% 里混着"分散投放 vs 集中押注"的效应，**不能全部归因到质量门禁**。要干净地隔离门禁的贡献，需要一条"同样分散买、但不看门禁"的第三臂——我没做。
...
4. **两臂的候选池不同。** 两臂共享同一批 `GateResult` 与同一套定向筛选（这是为了让差额归因干净），但 KOXPilot 臂只收 `pass`（`INCLUDE_REVIEW_BY_DEFAULT=False`），基线臂收全部三档。这是刻意的（基线就该不看门禁），代价是差额里包含"池子大小差异"这一项，我没有拆开。
5. **只有一个基线。** 没有"按 CPM 排序""按互动率排序"等更强对照，所以正确读法是"相比最粗暴的做法"，不是"相比专业买手"。
...
3. **强基线 + 归因隔离臂**：至少补"按 CPM 排序""按互动率排序"两条对照臂；更关键的是补一条**"同样按条分散买、但不看门禁"**的第三臂，才能把"质量门禁的贡献"和"分散投放的贡献"拆开（§7）。另外 3 个 campaign 的样本量给不出稳定结论，至少要 20+ 个 brief。
